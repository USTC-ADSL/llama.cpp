...
class ExportableAttentionCoreStage(nn.Module):
    ...
    def input_names(self) -> List[str]:
        return ["x", "qcur", "kcur", "vcur", "attn_bias", "cache_k", "cache_v"]
    ...
    def forward(
        ...
        queries = [qcur[:, i, :] for i in range(self.n_heads)]
        ...
        head_outs, _scaled_keys, _scaled_values = self.attention.core(
            queries,
            keys,
            key_t_caches,
            values,
            value_caches,
            attn_bias,
            kq_scale=(1.0 / math.sqrt(self.head_dim)),
        )

        fp16_inputs = [head_outs[i] for i in self.attention.fp16_head_ids]
        int4_inputs = [head_outs[i] for i in self.attention.int4_head_ids]

        outs = [x]
        if len(fp16_inputs) > 0:
            outs.append(self.attention.fp16_o_proj(fp16_inputs))
        if len(int4_inputs) > 0:
            outs.append(self.attention.int4_o_proj(int4_inputs))

        return sum(outs)


class AttentionCoreExporter:
    def __init__(self, graph_name: str, model_chunk: ExportableAttentionCoreStage, output_folder: Path):
...
def build_current_qkv(attention_module: nn.Module,
                      x: torch.Tensor,
                      rope_embeds: Tuple[torch.Tensor, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    attn_input = attention_module.norm(x)

    queries = [q_head(attn_input) for q_head in attention_module.q_heads]
    keys = [k_head(attn_input) for k_head in attention_module.k_heads]
    if getattr(attention_module, "use_qk_norm", False):
        queries = [attention_module.q_norms[i](query) for i, query in enumerate(queries)]
        keys = [attention_module.k_norms[i](key) for i, key in enumerate(keys)]

    queries = [attention_module.rope_qs[i](query, rope_embeds) for i, query in enumerate(queries)]
    keys = [attention_module.rope_ks[i](key, rope_embeds) for i, key in enumerate(keys)]
    values = [v_head(attn_input) for v_head in attention_module.v_heads]

    qcur = torch.stack(queries, dim=1).contiguous()
    kcur = torch.stack(keys, dim=1).contiguous()
    vcur = torch.stack(values, dim=1).contiguous()
...
def flatten_attn_core_inputs(attention_module: nn.Module, inputs: Tuple[object, ...]) -> Tuple[torch.Tensor, ...]:
    ...
    x_cpu = clone_cpu_tensor(x)
    rope_cpu = (clone_cpu_tensor(rope_embeds[0]), clone_cpu_tensor(rope_embeds[1]))
    qcur, kcur, vcur = build_current_qkv(attention_module, x_cpu, rope_cpu)

    cache_k = torch.stack([clone_cpu_tensor(t).transpose(0, 1).contiguous() for t in key_t_caches], dim=1)
    cache_v = torch.stack([clone_cpu_tensor(t).contiguous() for t in value_caches], dim=1)
...
if sorted(attn_core_modules.keys()) != selected_layers:
    missing = sorted(set(selected_layers).difference(attn_core_modules.keys()))
...
for model_chunk in model.model_chunks:
    for layer in model_chunk.layers:
        if layer.layer_id not in attn_core_modules:
            continue

        def save_sample(module, inputs, output, layer_ref=layer):
            if not inputs or inputs[0].shape[0] != graph_params.batch_size:
                return
...
print(f"Collected attention-core samples for {len(attn_core_modules)} layers")
...
for layer_id in selected_layers:
    graph_name = f"attn_core_layer_{layer_id}_{args.graph_name}"
    output_folder = args.output_folder / f"attn_core_layer_{layer_id}"
    output_folder.mkdir(parents=True, exist_ok=True)

    if not attn_core_modules[layer_id].saved_samples:
        raise RuntimeError(f"No attention-core calibration samples captured for layer {layer_id}")

    exporter = AttentionCoreExporter(
        graph_name=graph_name,
        model_chunk=attn_core_modules[layer_id],
    ...
    config["graphs"].append({
        "type": "attn_core",
        "graph_name": graph_name,
        "model_path": f"attn_core_layer_{layer_id}/attn_core_layer_{layer_id}.bin",
        "x_name": "x",
        "out_name": "out",
        "q_name": "qcur",
        "k_name": "kcur",
        "v_name": "vcur",
        "cache_k_name": "cache_k",
        "cache_v_name": "cache_v",
        "attn_bias_name": "attn_bias",
        "batch_size": graph_params.batch_size,
        "cache_size": graph_params.cache_size,
        "context_size": graph_params.context_size,
