...
class ExportableAttentionProjectionStage(nn.Module):
    ...
    def input_names(self) -> List[str]:
        return ["x", "rope_embed_cos", "rope_embed_sin"]
    ...
    def forward(
        ...
        rope_embeds = (rope_embed_cos, rope_embed_sin)
        attn_input = self.attention.norm(x)

        queries = [q_head(attn_input) for q_head in self.attention.q_heads]
        keys = [k_head(attn_input) for k_head in self.attention.k_heads]
        if getattr(self.attention, "use_qk_norm", False):
            queries = [self.attention.q_norms[i](query) for i, query in enumerate(queries)]
            keys = [self.attention.k_norms[i](key) for i, key in enumerate(keys)]

        queries = [self.attention.rope_qs[i](query, rope_embeds) for i, query in enumerate(queries)]
        keys = [self.attention.rope_ks[i](key, rope_embeds) for i, key in enumerate(keys)]
        values = [v_head(attn_input) for v_head in self.attention.v_heads]

        return (
            torch.cat(queries, dim=-1).contiguous(),
            torch.cat(keys, dim=-1).contiguous(),
            torch.cat(values, dim=-1).contiguous(),
        )


class AttentionProjectionExporter:
    def __init__(self, graph_name: str, model_chunk: ExportableAttentionProjectionStage, output_folder: Path):
    ...
    def export_quantization_config(self):
        ...
        encode_activation("rope_embed_cos", 32)
        encode_activation("rope_embed_sin", 32)
        encode_activation("qcur", 32)
        encode_activation("kcur", 32)
        encode_activation("vcur", 32)
...
def flatten_projection_inputs(inputs: Tuple[object, ...]) -> Tuple[torch.Tensor, ...]:
    if len(inputs) != 5:
        raise RuntimeError(f"expected 5 attention inputs, got {len(inputs)}")

    x, _key_t_caches, _value_caches, _attn_bias, rope_embeds = inputs
    if not isinstance(rope_embeds, tuple) or len(rope_embeds) != 2:
        raise RuntimeError("unexpected rope input structure")
...
projection_modules: Dict[int, ExportableAttentionProjectionStage] = {}
for model_chunk in model.model_chunks:
    for layer in model_chunk.layers:
        if layer.layer_id in selected_layers:
            projection_modules[layer.layer_id] = ExportableAttentionProjectionStage(
                layer_id=layer.layer_id,
                attention_module=layer.attn,
            )

if sorted(projection_modules.keys()) != selected_layers:
    missing = sorted(set(selected_layers).difference(projection_modules.keys()))
    raise RuntimeError(f"Missing attention modules for layers: {missing}")

hook_handles = []
for model_chunk in model.model_chunks:
    for layer in model_chunk.layers:
        if layer.layer_id not in projection_modules:
            continue

        def save_sample(module, inputs, _output, layer_id=layer.layer_id):
            if not inputs:
                return
            x = inputs[0]
            ...
            sample_inputs = flatten_projection_inputs(inputs)
            with torch.no_grad():
                proj_outputs = projection_modules[layer_id](inputs[0], inputs[4][0], inputs[4][1])
            sample_outputs = tuple(clone_cpu_tensor(t) for t in proj_outputs)
            projection_modules[layer_id].saved_samples.append(Sample(sample_inputs, sample_outputs))
...
print(f"Collected attention projection samples for {len(projection_modules)} layers")
...
for layer_id in selected_layers:
    graph_name = f"attn_proj_layer_{layer_id}_{args.graph_name}"
    output_folder = args.output_folder / f"attn_proj_layer_{layer_id}"
    output_folder.mkdir(parents=True, exist_ok=True)

    if not projection_modules[layer_id].saved_samples:
        raise RuntimeError(f"No attention projection calibration samples captured for layer {layer_id}")

    exporter = AttentionProjectionExporter(
        graph_name=graph_name,
        model_chunk=projection_modules[layer_id],
    ...
    config["graphs"].append({
        "type": "attn_proj",
        "graph_name": graph_name,
        "model_path": f"attn_proj_layer_{layer_id}/attn_proj_layer_{layer_id}.bin",
        "x_name": "x",
        "q_name": "qcur",
        "k_name": "kcur",
        "v_name": "vcur",
